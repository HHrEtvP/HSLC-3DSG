...
import model.modeling.detector.pointnet2.pointnet2_utils as pointnet2_utils
import model.modeling.detector.pointnet2.pytorch_utils as pt_utils
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            use_xyz: bool = True,
            pooling: str = 'max',
            sigma: float = 0.1, # for RBF pooling
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        new_xyz : torch.Tensor
            (B, npoint, 3) tensor of the new features' xyz
        new_features : torch.Tensor
            (B, \sum_k(mlps[k][-1]), npoint) tensor of the new_features descriptors，如果npoints取None，那么就返回(B,\sum_k(mlps[k][-1]))
        inds: torch.Tensor
        ...
        if self.npoint is not None:
            if inds is None:  # 默认是不应该提供的，而且在pointnet2原版里也没有提供，我有点操心的是，这里会不会因为npoints是None而报错，为了保险起见，如果这里的npointssh是None就直接跳过所有
                inds = pointnet2_utils.furthest_point_sample(xyz, self.npoint)
            else:
                assert(inds.shape[1] == self.npoint)
            new_xyz = pointnet2_utils.gather_operation(
            xyz_flipped, inds
            ).transpose(1, 2).contiguous() if self.npoint is not None else None  # 根据inds取采样点的坐标，如果npoints是Noned的话，这里new_xyz也应该是None
        else:
            new_xyz = xyz  # 在groupall时直接用原有的xyz就行了，反正也不需要

        if not self.ret_unique_cnt:
            grouped_features, grouped_xyz = self.grouper(  # group_all也只是需要xyz和features，根本就不需要new_xyz，所以是None也无所谓了
                xyz, new_xyz, features
            )  # (B, C, npoint, nsample)
        else:
            ...
            # 如果是group_all，返回(B, 3 + C, 1, N)

        new_features = self.mlp_module(
            grouped_features
        )  # (B, mlp[-1], npoint, nsample)或(B, mlp[-1], 1, N)(在这里，1是npoints，代表只有1组，N代表该组包含了所有的输入点)
        if self.pooling == 'max':  # 下面池化
            new_features = F.max_pool2d(
                new_features, kernel_size=[1, new_features.size(3)]
            )  # (B, mlp[-1], npoint, 1)或(B, mlp[-1], 1, 1)
        ...
        new_features = new_features.squeeze(-1)  # (B, mlp[-1], npoint)或(B, mlp[-1], 1)
        if new_features.shape[-1] == 1:
            new_features = new_features.squeeze(-1)  # 如果是(B, mlp[-1], 1)的情况就需要再squeeze一次
