...
import numpy as np
from model.modeling.detector.utils.pc_util import bbox_corner_dist_measure
...
def nms_3d_faster(boxes, overlap_threshold, old_type=False):
    ...
    while (I.size!=0):
        last = I.size
        i = I[-1]
        pick.append(i)  # 得分最高的BOX先加入

        # 用得分最高的BOX的右上和左下点的坐标去与场景内其余BOX对比，取得相交区域的边角点
        # (K-1,)
        xx1 = np.maximum(x1[i], x1[I[:last-1]])
        yy1 = np.maximum(y1[i], y1[I[:last-1]])
        ...
        if old_type:
            ...
            o = inter / (area[i] + area[I[:last-1]] - inter)
            '''
            如果两个BOX不相交，inter就是0，这个o自然就是0
            '''

        I = np.delete(I, np.concatenate(([last-1], np.where(o>overlap_threshold)[0])))
        # 从I中删去：被选中的BOX(last)，以及任何BOX与被选中BOX的IoU超过阈值的BOX，I是BOX的索引
