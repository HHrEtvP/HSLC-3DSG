...
from .utils_agcn import obj_edge_vectors, volume, sort_by_score, to_onehot, get_dropout_mask, nms_overlaps, \
...
class LSTMContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, obj_feauture_channels):
        ...
        self.agcn = aGCN(obj_dim=self.embed_dim + self.hidden_dim + self.obj_feature_dim,
                             rel_dim=self.embed_dim + self.hidden_dim + self.obj_feature_dim,
                             step=3)
        ...
        self.out_proj = torch.nn.Linear(self.embed_dim + self.hidden_dim + self.obj_feature_dim, self.hidden_dim)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        num_obj_batch = [len(p) for p in proposals]
        num_rel_batch = [r.shape[0] for r in rel_pair_idxs]
        obj_rel_rep = list(obj_rel_rep.split([len(proposal) for proposal in proposals]))
        X_obj, X_rel, obj2obj, sub2rel, obj2rel = self.getGraph(obj_rel_rep, rel_pair_idxs, proposals, obj_rel_rep[0].shape[-1], self.cfg)
        obj_out, rel_out = self.agcn(obj_feats_input=X_obj,
                             rel_feats_input=X_rel,
                             obj_obj=obj2obj,
                             sub_rel=sub2rel,
                             obj_rel=obj2rel)
        ...
        return obj_out, obj_preds, rel_out, obj_id_this_batch

    def getGraph(self, obj_node_feat, rel_pair_idxs, proposals, obj_feat_dim, cfg):
        num_box_per_scan = [len(proposal) for proposal in proposals]
        num_total_box = sum(num_box_per_scan)
        num_total_rel = sum([r.shape[0] for r in rel_pair_idxs])
        batch_size = len(proposals)
        device = torch.device('cuda:0')
        rel_feat_dim = obj_feat_dim
        obj2obj = torch.zeros((num_total_box, num_total_box), device=device)
        sub2rel = torch.zeros((num_total_box, num_total_rel), device=device)
        obj2rel = torch.zeros((num_total_box, num_total_rel), device=device)
        X_obj = torch.zeros((num_total_box, obj_feat_dim), device=device)
        X_rel = torch.zeros((num_total_rel, rel_feat_dim), device=device)
        obj_now, rel_now = 0, 0
        for i, (scan_pair, scan_nodes, num_box) in enumerate(zip(rel_pair_idxs, obj_node_feat, num_box_per_scan)):
            obj2obj_scan = torch.ones((num_box, num_box), device=torch.device('cuda:0')) \
                           - torch.eye(num_box, device=torch.device('cuda:0'))
            obj_feat_scan = torch.zeros((num_box, obj_feat_dim), device=torch.device('cuda:0'))
            # rel_feat_scan = torch.zeros((num_rel, rel_feat_dim), device=torch.device('cuda:0'))
            rel_feat_scan = self.rel_proj(torch.cat([scan_nodes[scan_pair[:, 0]], scan_nodes[scan_pair[:, 1]]], dim=-1))
            num_rel = scan_pair.shape[0]
            sub2rel_scan = torch.zeros((num_box, num_rel), device=torch.device('cuda:0'))
            obj2rel_scan = torch.zeros((num_box, num_rel), device=torch.device('cuda:0'))
            sub2rel_scan.scatter_(0, (scan_pair[:, 0].contiguous().view(1, -1)), 1)
            obj2rel_scan.scatter_(0, (scan_pair[:, 1].contiguous().view(1, -1)), 1)
            """
            for j, pair in enumerate(scan_pair):
                '''Object Node Feature'''
                obj_feat_scan[pair[0]] = scan_nodes[pair[0]]
                obj_feat_scan[pair[1]] = scan_nodes[pair[1]]
                '''Relation Node Feature'''
                rel_node = self.rel_proj(torch.cat([scan_nodes[pair[0]], scan_nodes[pair[1]]], dim=0))
                rel_feat_scan[j] = rel_node
                '''subj->rel&rel->obj'''
                sub2rel_scan[pair[0]][j] = 1
                obj2rel_scan[pair[1]][j] = 1
            """
            X_obj[obj_now: obj_now+num_box] = obj_feat_scan
            X_rel[rel_now: rel_now+num_rel] = rel_feat_scan
            obj2obj[obj_now: obj_now+num_box, obj_now: obj_now+num_box] = obj2obj_scan
            sub2rel[obj_now: obj_now+num_box, rel_now: rel_now+num_rel] = sub2rel_scan
            obj2rel[obj_now: obj_now+num_box, rel_now: rel_now+num_rel] = obj2rel_scan
            obj_now = obj_now+num_box
            rel_now = rel_now+num_rel
        return X_obj, X_rel, obj2obj, sub2rel, obj2rel
