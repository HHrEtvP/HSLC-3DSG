...
cfg.merge_from_file(os.path.join(ROOT_DIR, "e2e_relation_predcls_votenet_agcn.yaml"))
cfg.freeze()
...
def eval_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(EVAL_DATALOADER):
        ...
        def to_oneshot(labels, num_cat):
            num_entry = labels.shape[0]
            oneshot_label = torch.zeros((num_entry, num_cat), dtype=torch.int64, device=labels.device)
            oneshot_label.scatter_(-1, labels.unsqueeze(-1), 1)
            return oneshot_label

        refine_logits = []
        for label in obj_labels:
            refine_logits.append(to_oneshot(label, 26))
